	...
	public void initColeta(){
		...
		if (connect()){
			...
			try{				
				...
				this.jboss.setGerenciavel(false);
				this.servidorAplicacaoBO.updateServidorAplicacaoColeta(this.jboss);
				System.out.println("JBoss n�o gerenci�vel - "+ jboss.getHostname() + "- Erros durante a coleta ");
				ex.printStackTrace();
			}finally{
	...
	private boolean connect(){
		...
		try{
			...
			if (code == 200){
				...
				System.out.println("JBOSS Ind�spon�vel - "+ this.jboss.getHostname() +" - HTTP Status: "+code);
